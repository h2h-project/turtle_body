...
//  Forked from v1.0 SCADs/Bottle_Control_Cap_v1X.scad.
...
//  build/test.py or VERSION.json. If any change below is adopted for
...
//  further.
//
//  Deliberate deviations from v1X:
//   1. Bottle outside diameter used for the insert-fit chain is 86 mm here
//      (was 85 mm in v1X; vs. the project default 82 mm / p_bottle_d()).
//      This only changes the insert OD (the plug that enters the bottle
//      socket); the cap disk, button and axle-bore dimensions are untouched.
//   2. Two internal tabs are added inside the hollow insert cavity, rooted
//      at the cavity floor (where the hollow meets the solid roof/boss,
//      i.e. the "inside bottom" of the cap), 22 mm wide (tangential), and
//      projecting 2 mm radially inward from the cavity wall -- locally
//      narrowing the circular interior on those two sides. The tab runs the
//      full height of the cavity (floor to the top lip) and then continues
//      28 mm PAST that lip (was 14 mm in v1X, extended a further 14 mm
//      here), so each tab stands proud above the cap as a free post. The
//      pair is placed on the button axis (shaft-hole centre through both
//      button-hole centres) and then swung 12 deg CLOCKWISE (viewed from
//      above) off that axis together -- no longer symmetric on the axis.
//   3. Above a point 10 mm BELOW the lip, the INNER face leans inward
//      (radially) as a single continuous slope, gaining 6 mm (total, was
//      5 mm) by the free tip -- i.e. the taper now starts partway down the
//      rooted rib, not exactly at the lip, and runs 38 mm total (10 mm
//      inside the rib + the full 28 mm overhang): about a 9 deg lean off
//      vertical, shallow enough to print without supports (the outer face
//      and the two tangential side faces stay flat, only the inner face
//      slopes).
//   4. Centre axle bore diameter nudged 8.6 -> 8.7 mm (radial clearance to
//      the Ø8 axle round section: 0.3 -> 0.35 mm).
...
/* [Test bottle fit] */
test_bottle_diameter   = 86;   // overrides p_bottle_d() for the insert-fit chain only
...
/* [Axle and buttons] */
shaft_hole_d   = 8.7;  // was 8.6; radial clearance to the Ø8 axle now 0.35 mm (was 0.3)
button_upper_d = 17;
button_axis    = "y"; // [x,y]
...
band_channel_w = 2.7;      // groove axial height (was 2; matches the retuned
                            // silicone-ring mold: 2.6 mm ring + 0.1 mm margin)
...
tab_top_overhang = 28;  // tabs run the FULL cavity height, floor to top lip,
                         // then continue this far again PAST the top lip
                         // (was 14; extended a further 14 mm here)
tab_corner_fillet = 4;  // rounds the two bottom edges where the tab meets the floor
tab_tip_taper_gain = 6; // total inward (radial) lean of the inner face by the
                         // free tip vs. the plain rib thickness (was 5)
tab_taper_start = 10;   // taper begins this far below the lip (was 0 = at
                         // the lip); the taper now runs continuously over
                         // this 10 mm inside the rib PLUS the full 28 mm
                         // overhang = 38 mm total, ~9 deg off vertical for
                         // the 6 mm gain -- still a gradual, no-support lean
tab_rotate = -12;        // extra rotation of the tab pair about Z, off the
                         // button axis (the line through the shaft-hole
                         // centre and both button-hole centres): negative =
                         // clockwise, viewed from above (+Z looking down at
                         // the boss/insert side)

/* [Render] */
fn = 120; // [48:8:240]

// ---- derived: insert OD from the 86 mm test bottle ----------------------
test_bottle_socket_d = test_bottle_diameter - 2 * bottle_wall_t;              // 85
insert_shaft_d        = test_bottle_socket_d - 2 * insert_radial_clearance;   // 83
...
module cap_side_tabs(wall_d, outer_wall_d, roof_t, lip_h, tab_width, tab_depth, tab_overhang, axis, tab_fillet_r = 0, tab_taper_gain = 0, tab_taper_start_offset = 0, tab_rotate_deg = 0) {
