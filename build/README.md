...
python3 build/test.py             # mesh every render, compare bounds/warnings to the baseline
python3 build/baseline.py         # re-record the baseline (only when geometry changes are intentional)
...
4. `python3 build/test.py` — if a bounds change is intentional, re-run `baseline.py` and review the
   `tests/expected_bounds.json` diff as the sign-off.
